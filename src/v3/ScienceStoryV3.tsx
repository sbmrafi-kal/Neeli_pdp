...
const scenes: Scene[] = [
  {
    ...
    step: '01',
    navLabel: 'Formula',
    eyebrow: 'THE COMPLETE FORMULATION',
    title: 'Inside one drop, five ingredient actions.',
    body:
      'A dark coconut-oil base brings lauric acid together with Amla, Neeli, Karnasphota and Bhringraj—one multi-herb formula, not a single-botanical blend.',
  ...
  },
  {
    ...
    step: '02',
    navLabel: 'Fibre',
    eyebrow: 'RECOVERY PATH 1 · FIBRE',
    title: 'Lauric acid moves beyond the surface.',
    body:
      'Coconut oil’s lauric acid has a strong affinity for hair protein and is associated with fibre penetration—helping reduce protein loss from damaged and undamaged hair.',
  ...
  },
  {
    ...
    title: 'Three herbs meet the scalp environment.',
    body:
      'Amla contributes antioxidant potential. Neeli brings traditional scalp-care use. Karnasphota comes forward as the copper beat, with experimental extract research reporting activity involving TNF-alpha and nitric oxide.',
    ...
    evidence: [
      ...
      'Karnasphota · TNF-alpha + nitric-oxide activity reported experimentally',
    ],
  ...
  },
  {
    ...
    title: 'Bhringraj is studied where the cycle is signalled.',
    body:
      'In preclinical dermal-papilla research, Bhringraj has shown molecular activity associated with the anagen phase. This is a research view—not a claim that finished oil is physically delivered to the papilla.',
...
function SceneTrace({ scene }: { scene: Scene['key'] }) {
  return (
    <svg className={`sv3-trace sv3-trace--${scene}`} viewBox="0 0 100 100" aria-hidden="true">
      <defs>
        <marker id="sv3-arrow-gold" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
          <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="#cba135" />
        </marker>
        <marker id="sv3-arrow-lauric" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
          <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="#e8ddc7" />
        </marker>
        <marker id="sv3-arrow-amla" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
          <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="#91a667" />
        </marker>
        <marker id="sv3-arrow-neeli" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
          <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="#9184ad" />
        </marker>
        <marker id="sv3-arrow-karnasphota" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse">
          <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="#d17c59" />
        </marker>
      </defs>

      {scene === 'droplet' && (
        <path pathLength="1" d="M32 24 C55 12 82 32 74 68 C66 84 46 90 32 78" markerEnd="url(#sv3-arrow-gold)" />
      )}
      {scene === 'fibre' && (
        <>
          <path pathLength="1" d="M18 78 C36 68 46 54 58 42 C72 28 80 24 86 19" markerEnd="url(#sv3-arrow-lauric)" strokeWidth="0.75" />
          <circle cx="84" cy="20" r="4.5" stroke="#e8ddc7" strokeWidth="0.8" fill="none" className="sv3-target-pulse" />
        </>
      )}
      {scene === 'scalp' && (
        <>
          <path className="sv3-trace__amla" pathLength="1" d="M22 71 C40 69 49 62 58 49" markerEnd="url(#sv3-arrow-amla)" />
          <circle cx="58" cy="49" r="3.5" fill="#cca54e" className="sv3-target-pulse" />
          
          <path className="sv3-trace__neeli" pathLength="1" d="M18 80 C37 79 44 70 54 58" markerEnd="url(#sv3-arrow-neeli)" />
          <circle cx="54" cy="58" r="3.5" fill="#cca54e" className="sv3-target-pulse" />
          
          <path className="sv3-trace__karnasphota" pathLength="1" d="M82 84 C72 76 67 63 70 48 S80 28 85 18" markerEnd="url(#sv3-arrow-karnasphota)" />
          <circle cx="70" cy="48" r="3.5" fill="#d17c59" className="sv3-target-pulse" />
        </>
      )}
      {scene === 'follicle' && (
        <>
          <path pathLength="1" d="M53 26 L53 52" markerEnd="url(#sv3-arrow-gold)" strokeWidth="0.85" />
          <circle cx="53" cy="56" r="6.5" stroke="#cca54e" strokeWidth="1.2" fill="none" className="sv3-target-pulse" />
          <circle cx="53" cy="56" r="9.5" stroke="#cca54e" strokeWidth="0.6" strokeDasharray="2 2" fill="none" />
        </>
      )}
    </svg>
  )
  ...
  return (
    <article className={`sv3-copy sv3-copy--${scene.key}`} aria-hidden={hidden || undefined}>
      <header className="sv3-copy__eyebrow">
        <span className="sv3-copy__circle-num">{scene.step}</span>
        <p className="sv3-copy__eyebrow-text">{scene.eyebrow}</p>
      </header>
            ...
            <ol className="sv3-progress" aria-label="Science story progress">
              {scenes.map((scene, index) => (
                <li key={scene.key} aria-current={activeScene === index ? 'step' : undefined}>
                  <span className="sv3-progress-num">{scene.step}</span>
